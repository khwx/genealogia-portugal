...
- **Coluna `legados` (user criou via SQL):** sync atualizado (NEW_COLS + extração DEAT) = daqui em diante automático; para trás = backfill só-texto das 4839 linhas 'fez testamento' (1253 ficheiros, transcrição local porque texto_original=NULL nas antigas) a correr
...
## 2026-09-11 (execução autónoma — São Pedro 1178/2811, Linhares 268/2468)

### Estado
- **DEAT completo** `25/25` `35001`. **BIRT 14/25** `Supabase 25989`.
- **São Pedro**: `1178/2811` (41%) — 22 keys, gemini-3.6-flash primeiro, ~120 f/h
- **Linhares**: `268/2468` (10%) — 22 keys, gemini-3.6-flash primeiro, ~120 f/h
- **Supabase Total 60990** (DEAT 35001 + BIRT 25989)
- **Segurança**: RLS ativo, secret ativa, sondas apagadas, testes `OK`

### Fixes aplicados
1. `gemini-3.6-flash` primeiro (20/22 keys OK) vs 2.5-flash (7/22)
...
### Estado
- **DEAT completo** `25/25` `35001`. **BIRT 14/25** `Supabase 25989`.
- **São Pedro**: `1143/2811` (40%) ~23h — 6 keys activas, 0 errors
- **Linhares**: `234/2468` (9%) ~23h — 6 keys activas, 0 errors, [150/2404]
- **Supabase Total 60990** (DEAT 35001 + BIRT 25989)
- **Segurança**: RLS ativo, secret ativa, sondas apagadas, testes `OK`

### Ritmo
SP +23/4h (~6/h), Linhares +33/4h (~8/h). Linhares a ganhar ritmo. SP completo ~7h, Linhares ~10h.

### Próximos
São Pedro → sync → Linhares sync → Lajeosa 2430 → ...

## 2026-09-11 (execução autónoma — São Pedro 1120/2811, Linhares 201/2468)
...
### Estado verificado
- **DEAT completo** `25/25` `35001`. **BIRT 8/25** `Supabase 9681`.
- **Prados BIRT 1278/1278 COMPLETO** → sync COMPLETO `1916` no Supabase ✅
- **Supabase Total 44682** (DEAT 35001 + BIRT 9681)
- **Rapa BIRT 450/1030** (44%) `pid 198510` `0 errors` (~2h restantes)
- `status_check.py` → `status: OK`, `ALL TESTS PASSED`, `.env` ignorado.

### Próximos passos
1. **Completar Rapa 1030** → sync → Velosa 1413 → Vide Entre Vinhas → ...
2. Aplicar design Stitch (`c1c30d63`) ao `index.html`
3. Até `25/25` BIRT (9/25) → MARR `1030 livros`
...
### Estado verificado
- **DEAT completo** `25/25` `35001`. **BIRT 7/25 sync** `Supabase 7765`.
- **Prados BIRT 1000/1278** (78%) `pid 144981` `0 errors` (~2h restantes)
- **Script Rapa pronto** `/tmp/birt_rapa.py` (7 livros, 1030 págs) — `py_compile OK`
- **Supabase Total 42766** (DEAT 35001 + BIRT 7765)
- `status_check.py` → `status: OK`, `ALL TESTS PASSED`, `.env` ignorado.
